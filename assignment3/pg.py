...
class PG(object):
  ...
  """
    ...
    """
        ...
        self.observation_placeholder = tf.placeholder(tf.float32, shape=(None, self.observation_dim), name="o")
        if self.discrete:
            self.action_placeholder = tf.placeholder(
                tf.int32, shape=(None,), name="a_disc")
        else:
            self.action_placeholder = tf.placeholder(tf.float32, shape=(None, None,), name="a_cont")

        # Define a placeholder for advantages
        self.advantage_placeholder = tf.placeholder(tf.float32, shape=(None,), name="advtg")
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    """
        ...
        if self.discrete:
            action_logits = build_mlp(self.observation_placeholder, self.action_dim, scope, output_activation=None)
            logit_prob = tf.multinomial(action_logits, 1)
            self.sampled_action = tf.squeeze(logit_prob, axis=1)
            # TODO: maybe not take log
            self.logprob = - tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=self.action_placeholder, logits=action_logits)
        else:
          action_means = build_mlp(self.observation_placeholder, self.action_dim, scope, output_activation=None)
          log_std = tf.get_variable("log_std", shape=(self.action_dim), dtype=tf.float32, trainable=True)
          self.sampled_action = tf.random_normal([self.config.batch_size, self.action_dim], action_means, tf.exp(log_std))
    ...
    """
        ######################################################
        #########   YOUR CODE HERE - 1-2 lines.   ############
        adam_optimizer = tf.train.AdamOptimizer(learning_rate=self.config.learning_rate)
        gnv = adam_optimizer.compute_gradients(self.loss)
    ...
    """
        ...
        # TODO
        self.baseline = build_mlp(self.observation_placeholder, 1, scope, output_activation=None)
        self.baseline_target_placeholder = tf.placeholder(
          tf.float32, shape=(None,), name="baseline")
        # TODO
        loss = tf.losses.mean_squared_error(
          tf.squeeze(self.baseline), self.baseline_target_placeholder)
        adam_optimizer = tf.train.AdamOptimizer(learning_rate=self.config.learning_rate)
        gnv = adam_optimizer.compute_gradients(loss)
        self.update_baseline_op = adam_optimizer.apply_gradients(gnv)
        #######################################################
        #########          END YOUR CODE.          ############
        ...
        for path in paths:
            ...
            for reward in reversed(rewards):
                if i == 0:
                    ...
                    path_return = path_returns[i-1]
                gt = self.config.gamma * path_return + reward
                path_returns.append(gt)
                i += 1
            #######################################################
            #########          END YOUR CODE.          ############
            all_returns.append(path_returns[::-1])
    ...
    """
        ...
        #########   YOUR CODE HERE - 5-10 lines.   ############
        if self.config.use_baseline:
            baselines = self.sess.run(self.baseline, feed_dict={
                self.observation_placeholder: observations})
            for i in range(len(adv)):
                adv[i] = adv[i] - baselines[i][0]
        if self.config.normalize_advantage:
            mean = np.mean(adv)
            var = np.sqrt(np.var(adv))
            adv = np.divide(np.subtract(adv, mean), var)
        #######################################################
        #########          END YOUR CODE.          ############
        return adv
