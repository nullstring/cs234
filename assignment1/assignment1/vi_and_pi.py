...
def new_value(V, P, nS, nA, policy, gamma):
	"""Get the new value function given the old one.
	...
	"""
	V_next = np.zeros(nS)
	for state, v in enumerate(V):
		action = policy[state]
		V_next[state] = 0.0
		for (probability, nextstate, reward, terminal) in P[state][action]:
			print (state, action, probability, nextstate, reward, terminal)
			V_next[state] += probability * (reward + gamma * V[nextstate])
	return V_next

def max_diff(V0, V1):
	diff = 0
	for state, v0 in enumerate(V0):
		v1 = V1[state]
		diff = max(diff, abs(v1-v0))
	return diff
...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, max_iteration=1000, tol=1e-3):
	...
	while k == 0 or (k < max_iteration and max_diff(V, V_next) >= tol):
		V = V_next
		k += 1
		V_next = new_value(V, P, nS, nA, policy, gamma)

	return V_next


def q_evaluation(P, nS, nA, value_from_policy, policy, gamma):
	q = np.zeros((nS, nA))
...
def policy_from_q(q, nS, nA):
	...
	for state in range(nS):
		for action in range(nA):
			if q[state][action] >= q[state][policy[state]]:
				policy[state] = action
	return policy
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	"""
	q = q_evaluation(P, nS, nA, value_from_policy, policy, gamma)
	# print q
...
def policy_iteration(P, nS, nA, gamma=0.9, max_iteration=20, tol=1e-3):
	...
	policy: np.ndarray
	"""
	value_from_policy = np.zeros(nS)
	policy = np.zeros(nS, dtype=int)
	improved_policy = policy
	k = 0
	
	while k == 0 or (k < max_iteration and max_diff(policy, improved_policy) > 0):
		policy = improved_policy
		k += 1
		value_from_policy = policy_evaluation(P, nS, nA, policy, 0.9, 1000, tol)
		improved_policy = policy_improvement(P, nS, nA, value_from_policy, policy, 0.9)
